...
describe Dome do
  ...
  context 'DEV ecosystem' do
    ...
    it 'prints correct message to the stdout' do
      expect { secret_manager.secret_env_vars(secrets_config) }.to output("[*] Setting \e[0;32;49mTF_VAR_dev_common_secret\e[0m.\n[*] Setting \e[0;32;49mTF_VAR_dev_databricks_username\e[0m.\n").to_stdout
    end
  ...
  context 'error returned' do
    ...
    it 'prints correct error into the stdout' do
      expect { secret_manager.secret_env_vars(secrets_config) }.to output("\e[0;33;49m[!] Secrets Manager secret not found for 'dev_common_secret_value', so TF_VAR_dev_common_secret was not set.\e[0m\n").to_stdout
    end
    ...
    context 'AccessDeniedException' do
      ...
      it 'prints correct error into the stdout' do
        expect { secret_manager.secret_env_vars(secrets_config) }.to output("\e[0;33;49m[!] Access denied by Secrets Manager for 'dev_common_secret_value', so TF_VAR_dev_common_secret was not set.\e[0m\n").to_stdout
      end
    end
  end
end
