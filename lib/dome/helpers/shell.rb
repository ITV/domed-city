  ...
  module Shell
    ...
    def upload_kpi(product, environment, author, bucket_name, file_name)
      ...
      if object_exists.include? "dome_kpi.csv"
        ...
        version_id = command_output("aws-vault exec #{product}-prd-pe -- env -- aws s3api list-object-versions --bucket itv-core-terraform-kpi --prefix #{product}-infra/dome_kpi.csv | jq -r '.Versions[] | select(.IsLatest==true) | .VersionId'", "Failed to get version id").strip
